...
{
    public class PizzaFinalRepository : EFBaseRepository<AppDbContext, Domain.Identity.AppUser, Domain.PizzaFinal, DTO.PizzaFinal>, 
        IPizzaFinalRepository
    {
        public PizzaFinalRepository(AppDbContext dbContext) : base(dbContext, 
            new BaseMapper<Domain.PizzaFinal, DTO.PizzaFinal>())
        {
        }

        public async Task<IEnumerable<DTO.PizzaFinal>> GetAllAsync(Guid id, Guid? userId = null, bool noTracking = true)
        {
            var query = RepoDbSet
                .Include(pf => pf.Pizza)
                .AsQueryable();
        ...
        public async Task<DTO.PizzaFinal> FirstOrDefaultAsync(Guid id, Guid? userId = null)
        {
            var query = RepoDbSet
                .Include(pf => pf.Pizza)
                .Where(pf => pf.Id == id)
