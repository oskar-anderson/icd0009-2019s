...
{
    public class SharingItemRepository :  EFBaseRepository<AppDbContext,  Domain.SharingItem, DAL.App.DTO.SharingItem>, ISharingItemRepository
    {
        public SharingItemRepository(AppDbContext dbContext) : base(dbContext, new BaseDALMapper<Domain.SharingItem, DAL.App.DTO.SharingItem>())
        {
        }

        public async Task<IEnumerable<SharingItem>> AllAsync(Guid? userId = null)
        {
            var query = RepoDbSet
                .Include(si => si.Item)
