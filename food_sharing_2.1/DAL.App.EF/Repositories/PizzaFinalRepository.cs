...
{
    public class PizzaFinalRepository :  EFBaseRepository<AppDbContext,  Domain.PizzaFinal, DAL.App.DTO.PizzaFinal>, IPizzaFinalRepository
    {
        public PizzaFinalRepository(AppDbContext dbContext) : base(dbContext, new BaseDALMapper<Domain.PizzaFinal, DAL.App.DTO.PizzaFinal>())
        {
        }

        public async Task<IEnumerable<PizzaFinal>> AllAsync(Guid? userId = null)
        {
            var query = RepoDbSet
                .Include(pf => pf.Pizza)
                .AsQueryable();
