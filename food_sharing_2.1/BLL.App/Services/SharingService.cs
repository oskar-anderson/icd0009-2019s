...
{
    public class SharingService : BaseEntityService<ISharingRepository, IAppUnitOfWork, DAL.App.DTO.Sharing, BLL.App.DTO.Sharing>, ISharingService
    {

        public SharingService(IAppUnitOfWork unitOfWork) 
            : base(unitOfWork, new BaseBLLMapper<DAL.App.DTO.Sharing, BLL.App.DTO.Sharing>(), unitOfWork.Sharings)
        {
        }

        public async Task<IEnumerable<BLL.App.DTO.Sharing>> AllAsync(Guid? userId = null)
        {
            return (await ServiceRepository.AllAsync(userId)).Select(dalEntity => Mapper.Map(dalEntity));
        }

        public async Task<BLL.App.DTO.Sharing> FirstOrDefaultAsync(Guid id, Guid? userId = null)
        {
