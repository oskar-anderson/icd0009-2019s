        ...
        <div class="row">
            <div class="col-md-4">
                <form submit.trigger="onSubmit($event)">
                    ...
                    -->
                    <button @click="updateOnClick(getChoice)" type="button" class="btn btn-primary">Submit</button>
                </form>
            </div>
        </div>
...
@Component
export default class ChoiceEdit extends Vue {
    get getChoice(): IChoice {
        const choice = store.state.choice;
        const choiceNotFound : IChoice = {
            id: "",
            questionId: "",
            name: "",
            givesPoints: false,
        };
        return choice != null ? choice : choiceNotFound;
    };
    ...
    updateOnClick(Choice: IChoice): void {
        console.log("Clicked on updateOnClick button");
        store.dispatch('editChoice', Choice);
    ...
    mounted(): void {
        ...
        store.dispatch("getChoice", this.$route.params.id)
        console.log("mounted");
    }
